...
public class BM25FScoring {
	...
	public Map<String, Double> performRanking() {
		...
		for (int i = 0; i < this.files.length; i++) {
			...
			System.out.println(this.originalFiles[i].split("<HEADER>")[0].replaceAll("_", " ").toLowerCase().trim()+" with "+this.queryTerms[0].toLowerCase().trim());
			if (this.originalFiles[i].split("<HEADER>")[0].replaceAll("_", " ").toLowerCase().trim().equals(this.queryTerms[0])) {
				wBM25F += headerWeight;
				System.err.println("Header weight added for  " + this.originalFiles[i].split("<HEADER>")[0]);
			}
			//System.err.println(i + " is " + wBM25F + " ---->" + this.originalFiles[i] + "------->");
			//docxAndScore.put(this.originalFiles[i].split("<HEADER>")[1], wBM25F);
